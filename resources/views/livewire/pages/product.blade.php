    ...
    <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        <div class="lg:grid lg:grid-cols-2 lg:items-start lg:gap-x-8">
            <div class="flex flex-col-reverse">
                {{-- code lama untuk blade jadi satu namun akhir nya di pisah --}}
                <div class="mx-auto mt-6 hidden w-full max-w-2xl sm:block lg:max-w-none">
                    <div class="grid grid-cols-4 gap-6" aria-orientation="horizontal" role="tablist">
                        @foreach ($product->images as $image)
                            <button id="tabs-{{ $loop->index }}-tab-1" wire:key="{{ $loop->index }}"
                                ...
                                aria-controls="tabs-{{ $loop->index }}-panel-1" role="tab" type="button">
                                <span class="sr-only">Angled view</span>
                                <span class="absolute inset-0 overflow-hidden rounded-md">
                                    <img wire:click="setPreviewUrl({{ $image->id }})"
                                        src="{{ Storage::url($image->path) }}" alt="{{ $product->title }}"
                ...
                {{-- <div>
                        <div id="tabs-1-panel-1" aria-labelledby="tabs-1-tab-1" role="tabpanel" tabindex="0">
                            <img src="{{ Storage::url($previewUrl) }}" alt="Angled front view with bag zipped and handles upright."
            ...
            <div class="mt-10 px-4 sm:mt-16 sm:px-0 lg:mt-0">
                ...
                <div class="mt-6">
                    <livewire:components.product-varian-selector :product="$product" />
                    {{-- <livewire:components.variant-color-selector :product="$product" /> --}}
                    {{-- <livewire:components.variant-option-selector :product="$product" /> --}}
                </div>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white dark:bg-gray-800 shadow rounded">
                <main class="mx-auto prose p-6">
                    ...
                    <x-mary-carousel :slides="$this->getSlides()" class="h-96" />
                    {!! tiptap_converter()->asHTML($product?->content ?? '', toc: true, maxDepth: 4) !!}
