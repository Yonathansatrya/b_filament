    ...
    <x-authentication-card>
        ...
        <x-mary-form method="POST" action="{{ route('login') }}">
            ...
            <div>
                ...
                <x-input id="email" class="block mt-1 w-full" name="email" type="email" :value="old('email')" required
                    autofocus autocomplete="username" />
            </div>
            <div>
                <x-label for="password" value="{{ __('Password') }}" />
                <x-input id="password" class="block mt-1 w-full" name="password" type="password" :value="old('password')"
